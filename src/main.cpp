...
// Settings
const unsigned int SOURCE_WIDTH = 1280;
const unsigned int SOURCE_HEIGHT = 720;

// Camera
auto camera = Polyhedra::Camera(glm::vec3(0.0f, 0.0f, 5.0f));
...
int main() {
    ...
    // Compile Shader(s)
    auto program = Polyhedra::Program("demo/pbr.vs", "demo/pbr.fs");
...
void processInput(GLFWwindow* window) {
    ...
    if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS) {
        camera.ProcessKeyboard(Polyhedra::FORWARD, deltaTime);
    }
    if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS) {
        camera.ProcessKeyboard(Polyhedra::BACKWARD, deltaTime);
    }
    if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS) {
        camera.ProcessKeyboard(Polyhedra::LEFT, deltaTime);
    }
    if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS) {
        camera.ProcessKeyboard(Polyhedra::RIGHT, deltaTime);
    }
}
